...
::: {.column width="30%"}
![](mary-ryan-class.png){width="85%" fig-alt="Me and my husband in Bruges, Belgium"}
:::
::::
...
::: {.column width="30%"}
![](eugene.jpg){width="75%" fig-alt="Headshot of TA, Eugene Kahng"}
:::
::::
...
- How do we model a [trend over time]{.alert}, or the time it takes for something to happen?
   - Longitudinal/repeated measures data
...
- Review of ordinary linear regression, weighted least squares
...
- (Generally) how to build a mean model (i.e., confounding and precision variables) that answers your research question
...
I've made the following [full e-books]{.alert} available on the Canvas site:

- [*Foundations of Liner and Generalized Linear Models*]{.underline}. 2015. Agresti A. John Wiley & Sons. ISBN: 978-1-118-73030-0
...
- [*Applied Longitudinal Analysis*]{.underline}. 2011. Fitzmaurice GM, Laird NM, and Ware JH. John Wiley & Sons. ISBN: 9780470380277

- [*Survival Analysis: A Self-Learning Text*]{.underline}. 2016. Moore, DF. Springer International Publishing. ISBN: 9783319312439
:::

::: {.column width="50%" style="font-size: 90%;"}
I've also [scanned]{.alert} several chapters from books I could not find electronic copies of:

- [*Analysis of Longitudinal Data*]{.underline}, 2nd edition. 2002. Diggle PJ, Heagerty P, Liang K, and Zeger SL. Oxford University Press. ISBN: 9780198524847

- [*Data Analysis Using Regression and Multilevel/Hierarchical Models*]{.underline}. 2007. Gelman A and Hill J. Cambridge University Press. ISBN: 9780521686891

- [*Applied Survival Analysis Using R*]{.underline}. 2012. Kleinbaum DG and Klein M. Springer Science+Business Media. ISBN: 9781441966452
:::
::::
...
- Randomly assigned groups of 3-4 (available in "People" section of Canvas)
...
- Bi-weekly homeworks assigned at end of class, due on due-date at beginning of class
...
- Scheduled for Th 10/09
   ...
   - [*How do we feel about moving this to Tues 10/14?*]{.alert}
...
- [Components]{.alert}
   ...
   - Oral project presentation
      ...
      - Designed to get you feedback on main project highlights
...
- Validity of responses is not guaranteed -- takes some prior knowledge to flag the nuances
...
$\vec{Y}$ and $\boldsymbol{X}$ are [data]{.alert}, while $\vec{\beta}$ is a vector of [model parameters]{.alert}

$\vec{\epsilon}$ models the unexplained [variation]{.alert} in the outcome
...
Because our data is only a [sample]{.alert} from the entire [population of interest]{.alert}, we always have uncertainty in that estimate of $\beta$... We need to incorporate variance of our parameter estimate to quantify that uncertainty
...
- Then we can create a $(100-\alpha/2)$% [confidence interval]{.alert} for $\widehat{\beta}$:
   $$\widehat{\beta} \pm Z_{1-\alpha/2}\sqrt{\widehat{V}/N}$$
:::
...
An alternative/complementary strategy is to use [weighted least squares]{.alert} (WLS)
...
The weighted estimate of our regression coefficients is now:
$$\widehat{\beta} = (\boldsymbol{X}^T\boldsymbol{W}\boldsymbol{X})^{-1}\boldsymbol{X}\boldsymbol{W}\vec{Y}$$
