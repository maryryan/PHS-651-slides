...
- Email: [xxx@wisc.edu](mailto:xxx@wisc.edu)
...
What if instead of assuming $Var[\vec{Y}|\boldsymbol{X}] = \sigma^2\boldsymbol{I}$, we estimate a residual for each observation $i$: $\widehat{\epsilon}_i$?
...
- But what do we use for $\boldsymbol{W}$?
   ...
   - Common to have $w_i = 1/\widehat{\sigma_i^2}$
...
Robust variance is known as a [post hoc]{.alert} correction because it only affects *inference* of $\widehat{\beta}$, not how we *estimate* $\widehat{\beta}$
