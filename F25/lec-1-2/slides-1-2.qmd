...
-   Average within-cluster individual-level variation: $\sigma^2 = \frac{1}{m}\sum_{i=1}^m\text{var}_i(y_{ij})$
...
::: {.fragment}
   ...
   made up of:
         ...
         - The systematic variation is what we're [making assumptions about]{.alert}
:::
:::
...
- Assumes that [clusters]{.alert} are *randomly* varying around the population mean response: $E[b_{0i}]=0$
   ...
   - If you added groups as covariates instead, those would assume nothing about this
...
$$\widehat{\beta} \pm Z_{\alpha/2} \text{diag}\left(\sum_{i=1}^n \boldsymbol{X}_i^T\left\{Cov[\vec{Y}_i]\right\}^{-1}\boldsymbol{X}_i^T\right)^{-1}$$
:::
