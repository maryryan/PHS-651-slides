...
library(qrcode)
```
<h1> Lecture 1.2: Conditional models for correlated data </h1>
...
Slides found at: <https://maryryan.github.io/PHS-651-slides/F25/lec-1-2/slides-1-2>

:::

::: {.absolute bottom=175 left=50 width=250}
```{r qr code, echo=F, fig.height=2, fig.width=2}
plot(qr_code("https://maryryan.github.io/PHS-651-slides/F25/lec-1-2/slides-1-2"))
```
:::
...
- [Independent data]{.underline}: data where each observation of a variable are not meaningfully correlated with other observations of the same variable (given other covariates $\boldsymbol{X}$)

- [Clustered/Correlated data]{.underline}: data where each observation of a variable has some kind of relationship to another set of observations of that variable (given other covariates $\boldsymbol{X}$)

   - AKA: nested/multilevel/hierarchical data
   
   - Might expect measurements on [units within a cluster to be more similar]{.alert} than measurements on units in different clusters
...
[Is it a clustering variable or is it an exposure/adjustment variable?]{.alert}
...
- Exposure/adjustment variables tend to have a [limited]{.alert} number of groups
   - e.g., diagnosis groups
   - Many groups means fewer degrees of freedom to spend on estimating coefficients
   
<br> 

- This boundary can sometimes be fuzzy! See Clark & Linzer (2014) or McNeish (2023) for more discussion

---

## Recap: correlated/clustered data

The ICC is a way to measure the "clustered-ness" of the outcome: a measure of within- vs. between-cluster variation

-   Cluster-specific mean: $\bar{y}_i = \frac{1}{n_i} \sum_{j=1}^{n_i} y_{ij}$

-   Variation in cluster means: $\gamma^2 = \text{var}(\bar{y}_i)$

-   Average [within-cluster]{.underline} individual-level variation: $\sigma^2 = \frac{1}{m}\sum_{i=1}^m\text{var}_i(y_{ij})$

$$\begin{align*}\text{ICC} =  \frac{\gamma^2}{\gamma^2 + \sigma^2} &= \frac{\text{variation in cluster means}}{\text{variation in cluster means } + \text{individual-level variation}}\\
&\\
&= \frac{\text{variation in cluster means}}{\text{total variation}}\end{align*}$$

---

## Caveat: Is `Var(Y)` = Total Variation?
...
library(ICC)
set.seed(091025)
var_y <- icc_y <- rep(NA,1000)

for( i in seq(1000)){
   
   y <- epsilon <- gamma <- data_example <- NULL
   
   epsilon <- rnorm(100, mean=0, sd=1) # within-cluster individual error 
   gamma <- rep(rnorm(10, mean=0, sd=2), each=10) # cluster-level error
   
   y <- gamma + epsilon # create our variable y
   var_y[i] <- var(y)
   
   data_example <- as.data.frame(cbind(y, rep(seq(10),each=10)))
   
   icc_y[i] <- ICCest(V2, y, data=data_example)$ICC
}

# true var_y = 2^2 + 1^2 = 5
mean(var_y)
# true icc_y = 2^2/5 = 0.8
mean(icc_y)
```
...
- Could [add group as covariate]{.alert} in model... fixes the issue (and takes care of confounding) but violates parameters vs data size assumptions
...
- $Z_i$ is the random effects indicator for cluster $i$ (sometimes we omit this and just have $b_{0i}$ on its own)
...
- $\vec{Z}_i$ is the [random effects]{.alert} vector for cluster $i$ (dimensions: $n_i \times 1$)
...
- Mean outcome averaged over the entire sample (weighted by cluster size)
:::
:::
...
::: {.fragment}
   ...
   - Think of this as a function of the variables we need to [isolate the causal effect]{.underline} of our exposure of interest
   
   - Usually variables that we care about for testing hypotheses
:::
:::
...
::: {.fragment}
   ...
   $$\color{blue}{\vec{\xi}_i} = \color{red}{\vec{Z}_ib_{0i}}+\color{BlueViolet}{\vec{\epsilon}_i}$$
   made up of:
         i. Random effects
         
            - Random between-cluster variation: $\color{red}{\vec{Z}_i b_{0i}}$
            
         ii. Within-cluster variation: $\color{BlueViolet}{\vec{\epsilon}_i}$
:::
:::
...
- Instead of estimating all the cluster-level effects separately, we assume they come from a [common distribution]{.alert} and borrow information between clusters to come up with variance estimates
:::
...
- By estimating a global $\sigma^2_b$, this allows clusters with fewer observations to be more influenced by the rest of the data ("borrow" information), and clusters with more observations to be less influenced

   - Clusters with few observations have their intercepts pulled toward the global intercept $\beta_0$!
...
- Assumes that [clusters]{.alert} are *randomly* varying around the population mean response: $E[b_{0i}]=0$
   ...
   - If you added cluster IDs as covariates instead, those cluster-level slopes wouldn't assume anything about randomness
...
::: {.alert .fragment .absolute bottom=125 left=650}
...
::: {.absolute bottom=100 right=100 width=500}
![](panda_combo.png)
:::
...
- In LMEs, we generally use [restricted maximum likelihood (REML)]{.alert} estimation because MLE gives us biased estimates of the covariance components in small samples

<br>

- This means that we [cannot]{.alert} perform [likelihood ratio test]{.underline}-based inference for $\vec{\beta}$ when using REML!
...
$$\widehat{\beta} \pm Z_{1-\alpha/2} \text{diag}\left(\sum_{i=1}^n \boldsymbol{X}_i^T\left\{Cov[\vec{Y}_i]\right\}^{-1}\boldsymbol{X}_i^T\right)^{-1}$$
:::
...
Wait, I thought we usually like to use a $t$-distribution critical value for confidence intervals?

- Generally, yes

<br>

- This gets complicated in correlated data when we try to calculate the degrees of freedom: each observation doesn't contribute fully independent information, so what is $n$ really?

<br>

- Often we can just use a standard normal Z critical value

<br>

- For small sample sizes, we can use a Satterthwaite or Kenward-Roger approximation of the degrees of freedom
   - Available in SAS via `PROC GLIMMIX`
   - Available in R via `model_parameters()` function in the `parameters` package
...
- They assume cluster membership adds an [additional level of variation]{.alert} to the outcome that isn't accounted for by OLS modeling strategies
...
- To account for confounding, we need to add additional (cluster-level) fixed effect variables to the mean model [... BUT this will not work if we already have a cluster-level covariate in the fixed portion of our model]{.fragment}
...
radon_lme_ci <- confint(radon_lme)
summary(radon_lme)$coef
```

- LME $\widehat\beta_1$: `r round(summary(radon_lme)$coef[2,1],4)`

   - 95% CI (standard normal Z):  $(\widehat\beta_1 - Z_{0.975}SE_\beta, \widehat\beta_1 + Z_{0.975}SE_\beta)=$ (`r round(radon_lme_ci[4,1], 4)`, `r round(radon_lme_ci[4,2], 4)`)
...
How would these results differ if we calculated the 95% CI using a Satterthwaite approximation?
```{r radon lme satterthwaite}
library(parameters)

model_parameters(radon_lme, df_method="satterthwaite")
```

- Previous standard normal 95% CI: (`r round(radon_lme_ci[4,1], 4)`, `r round(radon_lme_ci[4,2], 4)`)
...
radon_cty_lm_ci <- confint(radon_cty_lm)
summary(radon_cty_lm)$coef[1:10,]
```
...
- OLS $\beta_1$: `r round(summary(radon_cty_lm)$coef[1,1],4)`
   ...
   - 95% CI: (`r round(radon_cty_lm_ci[1,1], 4)`, `r round(radon_cty_lm_ci[1,2], 4)`)
...
- LME $\beta_1$: `r round(summary(radon_lme)$coef[2,1],4)`
   ...
   - 95% CI: (`r round(radon_lme_ci[4,1], 4)`, `r round(radon_lme_ci[4,2], 4)`)
...
plot(NA, xlim=c(-0.9,-0.35), ylim=c(1,2.2), xlab="", ylab="", yaxt='n', cex.axis=1.5)
...
text(ci_lme[2,2]+0.08, 1.5, "LME CI", col="purple", cex=1.5)
```
:::
:::
...
::: {.fragment .absolute top=400 right=25 width="30%"}
