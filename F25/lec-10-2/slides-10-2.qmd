...
The KM estimator calculates a conditional survival probability for each time interval in which [only 1 event occurs]{.underline}, and then multiplies them together:
...
$$\log\left[\widehat \Lambda(t)\right] \pm Z_{1-\alpha/2} \times \sqrt{ Var\left[\widehat S(t)\right]/\left(\widehat S(t) \log\left[\widehat S(t)\right]\right)^2}  \Rightarrow\log\left[\widehat \Lambda(t)\right] \pm Z_{1-\alpha/2} \times \widehat{SE}_{\Lambda}(t)$$
...
If we use $\widehat S_{KM}(t)$ for every $\widehat S(t)$, and back-transform the above to be a confidence interval around $\widehat S_{KM}(t)$, we would get:
...
- It looks a little bit like a chi-squared test!
...
:::: {.columns}
::: {.column width="75%"}
![ ](Real-study-examples-on-non-proportional-hazards-NPH-patterns-a-Delayed-treatment.png){width=1200}
:::
