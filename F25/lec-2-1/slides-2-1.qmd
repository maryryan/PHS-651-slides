...
$$Y_{ij} = \color{red}{\beta_0 + U_{ij}\beta_1} + \color{blue}{Z_ib_{0i}}+\epsilon_{ij}$$
or in the more general matrix notation form: 
$$\vec{Y}_i = \color{red}{\boldsymbol{X}_i\vec{\beta}} + \color{blue}{\vec{Z}_ib_{0i}}+\vec{\epsilon}_i$$

- $Y_{ij}$ is the outcome for individual *j* in cluster *i*

- $U_{ij}$ is the exposure status for individual *j* in cluster *i*

- $\color{red}{\beta_0 + u_{ij}\beta_1}$ (or $\color{red}{\boldsymbol{X}_i\vec{\beta}}$) are the [fixed effects]{.underline} representing the mean responses for cluster $i$

- $\color{blue}{Z_i b_{0i}}$ ($\color{blue}{\vec{Z}_ib_{0i}}$) are the [random effects]{.underline} accounting for random between-cluster variation
...
Under a mixed model, the variance of a single outcome measure is:
$$\begin{align*}Var[Y_{ij}|\boldsymbol{X}_i] &= Var[(\beta_0 + U_{ij}\beta_1) + Z_ib_{0i}+\epsilon_{ij}|\boldsymbol{X}_i]\\
&= Var[Z_ib_{0i}+\epsilon_{ij}]\\
&= \sigma^2_b + \sigma^2_{\epsilon}\end{align*}$$ 

::: {.alert .absolute top=450 right=450}
(total variation!)
:::
...
$$\begin{align*}Cov[\color{red}{Y_{ij}}, \color{blue}{Y_{ik}}|\boldsymbol{X}_i] &= Cov[\color{red}{(\beta_0 + U_{ij}\beta_1) + Z_ib_{0i}+\epsilon_{ij}}, \color{blue}{(\beta_0 + U_{ik}\beta_1) + Z_ib_{0i}+\epsilon_{ik}}|\boldsymbol{X}_i]\\
&= Cov[\color{red}{Z_i b_{0i}+\epsilon_{ij}}, \color{blue}{Z_ib_{0i}+\epsilon_{ik}}]\\
&= Cov[\color{red}{Z_ib_{0i}}, \color{blue}{Z_ib_{0i}}] + Cov[\color{red}{\epsilon_{ij}}, \color{blue}{Z_ib_{0i}}] + Cov[\color{red}{Z_ib_{0i}}, \color{blue}{\epsilon_{ik}}] + Cov[\color{red}{\epsilon_{ij}}, \color{blue}{\epsilon_{ik}}]\\
&= \sigma^2_b + 0 + 0 + 0 \text{(since we assume random effects } b_{i0} \text{ are independent of individual error } \epsilon_{ij}\text{)}\\
&= \sigma^2_b\end{align*}$$

::: {.alert.absolute bottom=125 left=650}
In 552, we were used to assuming $Cov[Y_{ij}, Y_{ik}]=0$!
:::

---

## What does this covariance matrix look like (for observations in cluster *i*)? {.smaller}

$$\begin{align*}Cov[\vec{Y}_i|\boldsymbol{X}_i] &= \vec{Z}_i\sigma^2_b\boldsymbol{1}\vec{Z}_i^T + \sigma^2_\epsilon \boldsymbol{I}\\
...
- Known as a [block exchangeable]{.alert} matrix or [compound symmetry]{.alert}
   ...
   - Assumes everyone in the same cluster is similarly *exchangeable*, but assumes people from different clusters are *independent*

---

## What does this covariance matrix look like (for all observations)?

If you look at the *full* covariance matrix for all observations, it looks like a diagonal of these [block matrices]{.alert} with a bunch of 0's on the off diagonals

![From the SAS blog: <https://blogs.sas.com/content/iml/2012/11/07/constructing-block-matrices.html>](blockmatrix.png){width="40%"}
...
The [fixed effects]{style="color:red;"} are shared across everyone in the sample, while the [random effects]{style="color:blue;"} are unique to a particular cluster
...
## 552 review: logistic regression, independent observations

If our outcome for an individual $j$, $Y_{j}$, is [binary]{.alert}, we can model it using a logistic regression:
$$\log\left(\frac{p_j}{1-p_j}\right) = \beta_0 + U_j\beta_1 + \epsilon_j$$

- We assume $Y_j$ follows a binomial distrigution $\text{Binomial}(p_j)$ with event probability $p_j$
...
## 522 review: logistic regression, independent observations
...
1. Using our Binomial distribution assumptions to handle [non-constant variance]{.underline} in binary data
...
Say we want to look at the effect of an exposure variable $U$ on an outcome variable $Y$:

$$\begin{align*}g(Y_j) &= \beta_0 + U_j\beta_1 + \epsilon_j\\
&=\boldsymbol{X}_j\vec{\beta} + \epsilon_j\end{align*}$$
...
$$\sum_{j=1}^n \left(\frac{\partial \mu_j}{\partial \vec{\beta}}\right)^T v_j(\mu_j)^{-1}[Y_j - \mu_j(\vec{\beta})] = 0$$


where

- $\mu_j(\vec{\beta}) = E[Y_j]$ is the [mean of the outcome]{.underline} and is a function of our regression coefficients $\vec{\beta}$

- $v_j(\mu_j)$ is the part of the outcome variance $Var[Y_j]$ that depends on the mean -- this is determined by the distribution of our outcome $Y_j$

- [Bonus fact]{.underline}: The estimating equation is actually just the derivative of the likelihood with respect to regression parameters $\beta$ ($\partial f(y_j)/\partial \beta$)
...
$$\sum_{j=1}^n\boldsymbol{X}^T[Y_j - \boldsymbol{X}\vec{\beta}]=0 $$

. . .

OLS is a type of GLM!

- $E[Y_j] = \mu_j(\beta) = \boldsymbol{X}\vec{\beta} = \beta_0 + U_j\beta_1$

   - $\left(\frac{\partial \mu_j}{\partial \beta}\right) = \boldsymbol{X}$

- $v_j(\mu_j) = 1$ (the variance of $Y_j$ doesn't depend on the mean)
...
| Distribution | Var. function $v(\mu_j)$ | Usual link function                                                                     |
|--------------|------------------------|-----------------------------------------------------------------------------------|
| Normal       | $v(\mu_j)=1$             | Identity: $g(\mu_j) = \mu_j = \boldsymbol{X}_j\vec{\beta}$                            |
| Bernoulli/Binomial    | $v(\mu_j)=\mu_j(1-\mu_j)$    | Logit: $g(\mu_j) = \log\left(\frac{\mu_j}{1-\mu_j}\right)= \boldsymbol{X}_j\vec{\beta}$ |
| Poisson      | $v(\mu_j)=\mu_j$           | Log: $g(\mu_j) = \log(\mu_j)= \boldsymbol{X}_j\vec{\beta}$                            |


<br>

- We'll focus on binary/Bernoulli/Binomial data today and circle back to count/Poisson data in a recorded video
...
- Assume that $g(\cdot)$ is the identity function $g(\mu_j) = \mu_j = E[Y_j]$

- Model is then

$$g(E[Y_j]) = \mu_j = \boldsymbol{X}_i\vec{\beta} = \beta_0 + U_j\beta_1$$

a standard linear regression model

- Variance is $\text{Var}[Y_j] = \phi = \sigma^2$ 

   - $v(\mu_j) = 1$ (independent of mean)
...
- Assume that $g(\cdot)$ is the logit function $g(\mu_j) = \log\left(\frac{\mu_j}{1-\mu_j}\right)$

- Model is then

$$g(E[Y_j]) = \log\left(\frac{\mu_j}{1-\mu_j}\right) = \boldsymbol{X}_j\vec{\beta}$$

a logistic regression model

- Variance is $\text{Var}[Y_j] = v(\mu_j) = \mu_j(1-\mu_j)$ (*dependent* on mean)
...
$$g(Y_{ij}) = \beta_0 + U_{ij}\beta_1 + Z_ib_{0i}+\epsilon_{ij}$$

- Assumes our outcome $[\vec{Y}_i|\boldsymbol{X}_i, b_{0i}]$ is distributed as a member of the exponential family
...
- Say we have an outcome $Y_{ij}$ for individual *j* in cluster *i* that is distributed Bernoulli with probability of success $p_{ij}$

- We want to know what effect binary exposure $U_{ij}$ has on $Y_{ij}$
...
- We usually estimate the individual variation $Var[Y_{ij}|\boldsymbol{X}_i,b_{0i}]=Var[\epsilon_{ij}]$ as $p(1-p)$
...
[How do we interpret $\beta_0$ and $b_{0i}$ in this setting?]{.alert}

. . .

- $\exp{(\beta_0)}$: grand odds of the event occurring (in the reference group $u_{ij}=0$)

- $\exp{(\beta_0 + b_{i0})}$: cluster $i$-specific odds of the event occurring

- $\exp{(b_{i0})}$: odds ratio/relative change in odds of the event occurring in cluster $i$ compared to grand odds
...
Say we want to see whether [years of physician experience]{.alert} impacts whether a patient’s lung cancer goes into [remission]{.alert} ($Y_{ij}$)after treatment

- simulated fake data from [UCLA](https://stats.oarc.ucla.edu/r/dae/mixed-effects-logistic-regression/)

- physician treats multiple patients
...
- Each physician has different numbers of patients, with different total years of experience

- Pretty wide variation in the proportion of patients in remission for each physician

- Each physician treating patients in different cancer stages in different proportions
...
- Seeing a decent amount of remission clustering by physician!

---

## Example: cancer remission

Now let's see what a [non-clustered]{.alert} logistic regression would get us:
...
::: {.column width="70%"}
- $e^{\widehat\beta_1}$: `r round(exp(summary(logistic_fit)$coefficients[2,1]), 4)`

   - 95% CI: $\exp{(\widehat\beta_1 \pm Z_{0.975}\times SE_{\beta_1})}=$(`r round(exp(logistic_fit_ci[2,1]), 4)`, `r round(exp(logistic_fit_ci[2,2]), 4)`)
...
- $e^{\beta_1}$: `r round(exp(summary(glmm_fit)$coefficients[2,1]), 4)`
   ...
   - 95% CI: $\exp{(\widehat\beta_1 \pm Z_{0.975}\times SE_{\beta_1})}=$(`r round(glmm_fit_ci_l, 4)`, `r round(glmm_fit_ci_u, 4)`)
   
   - [Interpretation]{.alert}: [Comparing 2 cancer patients being treated by the same physician, patients have an estimated 11.35% increased odds of going into remission if their physician has 1 year of experience compared to the same physician with 0 years experience, cancer stage held constant. We are 95% confident the true odds is between `r (round(glmm_fit_ci_l, 4)-1)*100` and `r (round(glmm_fit_ci_u, 4)-1)*100`%.]{.fragment}
...
- The [logit]{.alert} link is very common
   ...
   - Gives us [odds ratios]{.underline} and [log-odds]{.underline} ratios for $\beta_1$

- The [probit]{.alert} link is sometimes used in certain fields for prediction
   
   - Function is the inverse Normal CDF
   
   - Coefficients are usually pretty difficult to interpret

- The [log]{.alert} link and [identity]{.alert} links also get used

   - The former gives us [risk ratios]{.underline} for $\beta_1$, while the latter gives us [risk differences]{.underline} for $\beta_1$

. . .

In this case, we choose our link function based on 2 criteria:

1. What [quantity]{.underline} are we actually wanting to estimate?
...
$$\text{logit}(Y_{ij}) = \beta_0 + U_{ij}\beta_1 + Z_ib_{0i} + \epsilon_{ij}$$
$$\text{logit}(\vec{Y}_{i}) = \boldsymbol{X}_i\vec{\beta} + \vec{Z}_ib_{0i} + \vec{\epsilon}_{i}$$

. . .

- The conditional mean of our outcome is then:
$$\text{logit}(\mu_{ij})=\text{logit}(E[Y_{ij}|\boldsymbol{X}_i, b_{0i}]) = \beta_0 + U_{ij}\beta_1 + Z_ib_{0i}$$
...
$$E[\vec{Y}_i|\boldsymbol{X}_i, b_{0i}] = \boldsymbol{X}_i\vec{\beta} + \vec{Z}_ib_{0i} ~~~~~~ E[Y_{ij}|\boldsymbol{X}_i, b_{0i}] = \beta_0 + U_{ij}\beta_1 + Z_ib_{0i}$$
...
::: {.column width="50%"}
$$\begin{align*}E[Y_{ij}|\boldsymbol{X}_i] &= E\left\{E[Y_{ij}|\boldsymbol{X}_i, b_{0i}]\right\}\\
&=E\left[\beta_0 + U_{ij}\beta_1 + Z_ib_{0i}\right]\\
&=\beta_0 + U_{ij}\beta_1\end{align*}$$
:::
::::
...
## Caveat: Some notes on estimation
...
## Caveat: Software using Gassuain quadrature/Laplace approximation
...
## Caveat: Troubleshooting convergence issues
...
## Caveat: Comparing results between SAS and R
...
pql_fit <- glmmPQL( remission ~ Experience + CancerStage,
         random = ~ 1 | doc_ID, family = binomial, data = cancer, verbose = F )
...
- Next class (Thurs 9/18) will be a hands-on activity day -- be sure to bring your laptop!

   - Can also participate on Zoom

- Small Assignment 1 (reading + discussion post) is due tonight at 11:59p

- I'll post the correlated count data video Wednesday night -- it'll extend what we've done today to count outcomes

   - You'll be able to do the in-class activity without watching this video