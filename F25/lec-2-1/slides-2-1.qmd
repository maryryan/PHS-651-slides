...
- $\beta_0 + u_{ij}\beta_1$ (or $\boldsymbol{X}_i\vec{\beta}$) are the [fixed effects]{.alert} representing the mean responses for cluster $i$

- $Z_i b_{0i}$ ($\vec{Z}_ib_{0i}$) are the [random effects]{.alert} accounting for random between-cluster variation

- $\epsilon_{ij}$ ($\vec{\epsilon}_i$) is the regular between-individual variation
   - or an individual's variation from the predicted outcome $\widehat{Y}_{ij} = E[Y_{ij}|\boldsymbol{X},b_{0i}]=\beta_0 + U_{ij}\beta_1$
   - $\epsilon_{ij} = (Y_{ij} - \widehat{Y}_{ij})^2$
...
$$\log\left(\frac{p_j}{1-p_j}\right) = \boldsymbol{X}_j\vec{\beta} + \epsilon_j = \beta_0 + U_j\beta_1 + \epsilon_j$$
...
$$\begin{align*}g(Y_j) &= \boldsymbol{X}_j\vec{\beta} + \epsilon_j\\
&= \beta_0 + U_j\beta_1 + \epsilon_j\end{align*}$$
...
- The mean of the outcome $E[Y_j]=\mu_j(\vec{\beta})$ is a function of our model parameters $\vec{\beta}$
...
| Distribution | Var. function $v(\mu)$ | Usual link function                                                                     |
|--------------|------------------------|-----------------------------------------------------------------------------------|
| Normal       | $v(\mu)=1$             | Identity: $g(\mu) = \mu = \boldsymbol{X}_j\vec{\beta}$                            |
| Bernoulli/Binomial    | $v(\mu)=\mu(1-\mu)$    | Logit: $g(\mu) = \log\left(\frac{\mu}{1-\mu}\right)= \boldsymbol{X}_j\vec{\beta}$ |
| Poisson      | $v(\mu)=\mu$           | Log: $g(\mu) = \log(\mu)= \boldsymbol{X}_j\vec{\beta}$                            |
...
- Say we have an outcome $Y_{ij}$ that is distributed Bernoulli with probability of success $p_{ij}$

- We want to know what effect exposure $U_{ij}$ has on $Y_{ij}$
