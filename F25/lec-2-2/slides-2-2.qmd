...
<h3>Online video: September 16-18 2025</h3>
...
1. [Stationarity]{.underline}: The expected number of events per unit time is the same throughout the entire time interval

2. [Independence]{.underline}: If an event occurs (or does not occur) in one time subinterval, it has no bearing on the probability of an event in the next time subinterval

3. [Rare events]{.underline}: For any very small time interval $h$:
...
- $T_j$ is known as an [offset]{.alert} $\rightarrow$ this allows us to account for different observation lengths or population totals that the events could occur in
...
- [School-level variables]{.underline}: school ID, school type (public/private)
...
- All data is measured on the same time-scale (annual) and has the same total population denominator (awards within a person)
   ...
   - We observe different numbers of students in each `cid` in this dataset, but offset is defined by the smallest observation unit
