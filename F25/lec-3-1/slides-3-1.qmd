...
::: {.column width="50%"}
![*Figure modified from Harrison et al. (2018). "A brief introduction to mixed effects modelling and multi-model inference in ecology." Ecology.*](random-intercept.png){width="120%"}
:::
...
- Recall that with LMEs and GLMMs, we modeled our [systematic variation]{.underline} (random effects and within-cluster variation) as:
$$\vec{b}_0 \sim N(\vec{0}, \boldsymbol{G}),~~~ \vec{\epsilon}_i \sim N(\vec{0}, \boldsymbol{R}_i=\sigma_\epsilon^2\boldsymbol{I})$$
