...
<br>
Interpretations of $\beta_1$ in a GLMM are more strictly [conditional]{.alert} because
...
- When using $\boldsymbol{G}=0$, we call this modeling with [R-side variance]{.alert}
...
- With OLS and LMEs/GLMMs, we had a likelihood-based estimating equation we could use to derive estimates for $\beta$
