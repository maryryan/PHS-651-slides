...
4. Comparing conditional and marginal effects from GLMMs and GEEs example
...
Last week we talked about how we can use [generalized estimating equations]{.alert} to fit [marginal models]{.alert}
...
- This is because we can collapse/simplify the conditional effect into the marginal effect when we are not transforming our outcome
   ...
   - This largely has to do with how $E[Y_{ij}|\boldsymbol{X},b_{0i}] = E[\boldsymbol{X}\vec{\beta} + Z_i b_{0i}] = E[\boldsymbol{X}\vec{\beta}] + E[Z_i b_{0i}] = E[\boldsymbol{X}\vec{\beta}] + 0$ (*we can take the expectation on each of those little addition portions separately*)
...
When we transform our outcome in a GLMM with some non-identity [link function]{.alert} $g(\cdot)$,
$$\begin{align*}g(E[Y_{ij}]) &= \boldsymbol{X}\vec{\beta} + Z_i b_{0i} + \epsilon_{ij}\\
&= \beta_0 + U_{ij}\beta_1 + Z_ib_{0i} + \epsilon_{ij}\end{align*}$$
...
- This is because we first need to [undo our transformation]{.underline} $g(\cdot)$ in order to take the expectation we want -- the transformation means we [can't easily separate]{.underline} the fixed and random effects to take the expectation of each separately like we did before

   - Layered water & oil vs shaken vinaigrette 
...
(i) the [marginal variance]{.underline} of the outcome: $var[Y_{ij}] = \phi v(\mu_{ij})$

    - Can sometimes be useful to separate this into $var(\mu_{ij})$ (the part of the variance that's related to $E[Y_{ij}]$) and $\phi$ (the part of the variance that's independent of $E[Y_{ij}]$)
    
    - But mostly we're just thinking about generally what form are we assuming for $var[Y_{ij}]$ 
...
2. Individuals in different clusters are [independent]{.underline}

3. Our covariance model is [*roughly* correct]{.alert} ($var[Y_{ij}]$ and the correlation structure)
...
- If we are using [robust/sandwich]{.underline} standard errors to make 95% CIs: we want to try to get close to the true covariance model, but some misspecification is okay because the robust standard error will correct it
...
3. Individuals in different clusters are [independent]{.underline}

4. Our outcome $Y_{ij}$ follows the [probability distribution]{.alert} implied by the LME/GLMM
...
- What I want you to get out of this is *generally* what each of the $\color{red}{\text{red}}$ and $\color{olive}{\text{green}}$ portions of that equation are doing (what is their general role), and how they change when we transform our outcome $Y_{ij}$

- You don't have to follow all the math, but hopefully this will make the differences in the types of GEEs we fit for different types of outcome data more clear

---

## The generalized estimating equation

Recall, when we fit a GEE, we're finding the regression coefficients $\widehat{\vec{\beta}}$ that solve an estimating equation of the
general form:
$$\sum_{i=1}^N \left(\color{red}{\frac{\partial \mu_i}{\partial \vec{\beta}}}\right)^T \color{olive}{V_i^{-1}}[y_{ij} - \color{red}{\mu_{ij}}] = 0$$

where

$$g(\color{red}{\mu_{ij}}) = g(E[Y_{ij}|\boldsymbol{X}]) = \boldsymbol{X}\vec{\beta} = \beta_0 + U_{ij}\beta_1~~~\text{(the mean model)}$$
...
$$g(\mu_{ij}) = \mu_{ij} = E[Y_{ij}|\boldsymbol{X}] = \boldsymbol{X}\vec{\beta} = \beta_0 + U_{ij}\beta_1$$
...
$$\color{red}{\frac{\partial \mu_{ij}}{\partial \vec{\beta}} = \frac{\partial \boldsymbol{X}\vec{\beta}}{\partial \vec{\beta}} = \boldsymbol{X}}$$
...
because $g(\mu_{ij}) = g(E[Y_{ij}|\boldsymbol{X}]) = \boldsymbol{X}\vec{\beta}$

- This means that $\mu_{ij} = g^{-1}(\boldsymbol{X}\vec{\beta})$, where $g^{-1}(\cdot)$ is the function that "undoes" $g(\cdot)$

- For example, if the link function is the log, $g(x) = \log(x)$, then $g^{-1}(x) = e^x$
...
- When modeling count outcomes, we usually use a log transformation: $g(\mu_{ij})=g(E[Y_{ij}|\boldsymbol{X}]) = \log(E[Y_{ij}|\boldsymbol{X}])$
...
- Along with our correlation structure, this would go into defining the $\color{olive}{\text{working covariance } V^{-1}}$
...
- $g(\mu_{ij}) = g(E[Y_{ij}]) = logit(E[Y_{ij}|\boldsymbol{X}]) = \boldsymbol{X}\vec{\beta}$, so $\mu_{ij} = g^{-1}(E[Y_{ij}]) =  expit(\boldsymbol{X}\vec{\beta})$

- $var[Y_{ij}] = g^{-1}(\mu_{ij})(1-g^{-1}(\mu_{ij})) = expit(\boldsymbol{X}\vec{\beta})[1-expit(\boldsymbol{X}\vec{\beta})]$ (like with Binomial data)

Our estimating equation then looks like:
...
I won't do the derivative of the $expit(\cdot)$ function because it's kind of gross, but hopefully it's clear how [transforming]{.underline} our outcome also changes how we're estimating the regression coefficients $\vec{\beta}$
...
$$g(\mu_{ij}) = \mu_{ij} = E[Y_{ij}|\boldsymbol{X}] = \boldsymbol{X}\vec{\beta}$$
...
- In our data, we observe multiple patients per physician
...
- Mean model: $logit\left(E[\text{remission}_{ij}|\boldsymbol{X}]\right) = \beta_0 + \beta_1(\text{experience}_i) + \beta_2(\text{Stage2}_{ij}) + \beta_3(\text{Stage3}_{ij}) + \beta_4(\text{Stage4}_{ij})$

- Variance: $Var[Y_{ij}] = E[\text{remission}_{ij}|\boldsymbol{X}]\left(1-E[\text{remission}_{ij}|\boldsymbol{X}]\right)$
...
| Sample size requirements   | Moderately large number of clusters ($n > 50$)                                            | Works with smaller sample sizes/number of clusters provided that the entire model is correctly specified |
...
With these differences in mind, let's see how GEEs and GLMMs compare with our cancer remission example
...
$$\text{logit}(\text{remission}_{ij}) = \beta_0 + \beta_1(\text{experience}_i) + \beta_2(\text{Stage2}_{ij}) + \beta_3(\text{Stage3}_{ij}) + \beta_4(\text{Stage4}_{ij}) + (Dr_i)b_{0i} + \epsilon_{ij}$$
...
- Went from an effect size of `r round((exp(cancer_glmm$coefficients$fixed[2])-1)*100, 2)`% higher odds of remission for each extra year of experience in the GLMM to `r round((exp(summary(cancer_gee)$coef[2,1])-1)*100,2)`% higher odds in the GEE. [Why?]{.alert}
:::
:::
::::
...
Let's try this again, but with our Minnesota radon data, where we're using log radon as our outcome (and are not interested in transforming measurements back to regular radon units)
...
Recall our GEE model from lectures 3.1 and 3.2
...
$$\text{log radon}_{ij} = \beta_0 + \beta_1(\text{floor}_{ij}) + (county_i)b_{0i} + \epsilon_{ij}$$
...
- Point estimates are approx. the same between the LME and the GEE and the model-based CIs are also similar

- We do get a slightly narrower CI using the robust standard errors for the GEE
