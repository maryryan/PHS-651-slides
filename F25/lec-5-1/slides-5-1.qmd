...
In mixed models, we also have conditional residuals (AKA level-1 residuals):
...
- Plots of [conditional]{.alert} residuals will tell you about how well the data fits your assumptions about the random intercepts (zero mean), the constant $\epsilon_{ij}$ variance assumption, and whether you have outlying subjects/clusters

- Plots of the [marginal]{.alert} residuals will tell you about the adequacy of the fixed effects (appropriate functional form/transformation)
:::
...
:::: {.columns}
::: {.column width="50%"}
```{r marg resid plot redres,eval=F}
library(redres)

# get various standardized and marginal residuals #
radon_resid_redres <- hlm_resid(radon_lme,standardize=TRUE)
...
::: {.column width="50%"}
```{r marg resid plot2 redres}
library(redres)
plot_redres(radon_lme, type="std_mar")
plot_resqq(radon_lme)
```
:::
::::
