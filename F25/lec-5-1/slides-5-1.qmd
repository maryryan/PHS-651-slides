...
- So we'll mostly be focusing on [mixed models]{.underline} in this session
...
- And [conditional residuals]{.alert} (subtracting marginal and random effects):
...
- We can also perform diagnostics by fitting multiple models with different assumptions and compare them on model fit $\rightarrow$ we'll get to that after the midterm
