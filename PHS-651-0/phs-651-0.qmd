...
- $Y_i$: (continuous) outcome/dependent variable for individual $i$
...
$$\vec{Y} = \boldsymbol{X\beta} + \vec{\epsilon},$$
where
...
$$E[\vec{Y}] = \boldsymbol{X\beta}$$

. . .

We then usually assume 
$$\vec{\epsilon} \sim N(0, \sigma^2)$$
---

## Review: OLS estimation

- Model parameters are "truth" that tell us the relationship between our independent variables and our response
   - We try to estimate what that relationship might be using data
...
- Because our data is only a [sample]{.alert}, we always have uncertainty...
