...
- In practice, the linear predictor is usually specified in terms of the mean $\lambda(u_i) T_i$ rather than the rate $\lambda_i$

$$\log\left(\lambda(u_i)\right) + \log(T_i) = \vec{X}_i\vec{\beta} + \log(T_i)$$
