...
<h1>Lecture 10: Time-to-event data</h1>
...
Slides found at: <https://maryryan.github.io/PHS-651-slides/PHS-651-10/slides-10>

:::

::: {.absolute bottom=0 left=100 width=150}
```{r qr code, echo=F, fig.height=2, fig.width=2}
plot(qr_code("https://maryryan.github.io/PHS-651-slides/PHS-651-10/slides-10"))
```
:::
...
A different type of longitudinal question, though, may instead ask about the [time]{.alert} it takes for individuals to experience a particular [event]{.alert}
...
The first occurs because we can't observe people for every second of every day [of their whole lives]{.alert}
   
1. [Left truncation]{.alert}: when, for some or all participants, there is a delay between the time origin of the study and the entry time
...
The second occurs because we can't observe people for every second of every day [of the study]{.alert}
...
- [Right censoring]{.alert}: the event is known to have occurred *after* a certain observation time, but we don't know when
...
- [Left censoring]{.alert}: an event is known to have occurred *before* a certain observation time, but we don't know when
...
[We can have censoring and truncation at the same time!]{.alert}

---

## An example

[We don't know when we're enrolling people in their event cycle!]{.fragment}
...
- [Subject 2: enrolled directly after transplant, observed for full study time (12 months); unobserved death at month 21. [Fixed right censoring]{.alert}]{.fragment}
...
- $T^*$: someone's true event time

- $X$: the time at which someone is censored/lost to follow up (censoring time)
