...
$$\log\left(\frac{p_i}{1-p_i}\right) = \boldsymbol{X}_i\vec{\beta} = \beta_0 + U_i\beta_1 + \epsilon_i$$
...
- The logit function $\log\left(\frac{x}{1-x}\right)$ acts as the [link]{.alert} between $E[Y_i]$ and the linear model $\boldsymbol{X}_i\vec{\beta}$
   ...
   - $\left(\frac{p_i}{1-p_i}\right)$ are known as the [odds]{.alert} of $u=1$ compared to $u=0$
...
$$\beta_1 = \log\left(\frac{p(u=1)}{1-p(w-1)}\right) - \log\left(\frac{p(u=0)}{1-p(u=0)}\right)$$
...
::: {.r-stack text-align="left"}

::: {.fragment .fade-out fragment-index=2}
where

- The mean of the outocme $E[Y_i]=\mu_i(\beta)$ is a function of $\beta$

- The variance of the outcome $Var[Y_i] = v_i$ is some function $v_i$
...
- $E[Y_i] = \mu_i(\beta) = \boldsymbol{X}\vec{\beta}$
   ...
   - $\left(\frac{\partial \mu_i}{\partial \beta}\right) = \boldsymbol{X}$
...
$$g(\vec{Y}_i) = \boldsymbol{X}_i\vec{\beta} + \vec{Z}_ib_{0i}+\vec{\epsilon}_i$$
   ...
   - Assumes $[\vec{Y}_i|\boldsymbol{X}_i, b_{0i}]$ is distributed as a member of the $\text{exponential family}$
   
   - Use distribution assumption to assume link function $g(\cdot)$ and the form of within-cluster variance
...
Our (loose) structural model is then:

$$\text{logit}(\text{remission}_{ij}) = \beta_0 + \beta_1(\text{experience}_i) + \beta_2(\text{Stage2}_{ij}) + \beta_3(\text{Stage3}_{ij}) + \beta_4(\text{Stage4}_{ij})$$
...
```{r cancer logistic}
logistic_fit <- glm( remission ~ Experience + CancerStage,
...
- $e^{\beta_1}$: `r round(exp(summary(logistic_fit)$coefficients[2,1]), 4)`
   ...
   - 95% CI: (`r round(exp(summary(logistic_fit)$coefficients[2,1] - summary(logistic_fit)$coefficients[2,2]*pnorm(0.975)), 4)`, `r round(exp(summary(logistic_fit)$coefficients[2,1] + summary(logistic_fit)$coefficients[2,2]*pnorm(0.975)), 4)`)
...
- Assumptions: correct structural model; [independence]{.alert}
...
```{r cancer glmm}
library(MASS)
glmm_fit <- glmmPQL( remission ~ Experience + CancerStage,
         random = ~ 1 | doc_ID, family = binomial, data = cancer, 
         verbose = F )
summary( glmm_fit )$tTable
...
- $e^{\beta_1}$: `r round(exp(glmm_fit$coefficients$fixed[2]), 4)`

   - 95% CI: (`r round(exp(glmm_fit$coefficients$fixed[2] - summary(glmm_fit)$tTable[2,2]*pnorm(0.975)), 4)`, `r round(exp(glmm_fit$coefficients$fixed[2] + summary(glmm_fit)$tTable[2,2]*pnorm(0.975)), 4)`)
...
- The logit link is very common

- The probit link is sometimes used in certain fields

- The identity link also gets used

---

## Count outcomes and GLMMs
