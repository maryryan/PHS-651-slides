...
$$\sum_{i=1}^n \left(\frac{\partial \mu_i}{\partial \vec{\beta}}\right)^T v_i^{-1}[Y_i - \mu_i(\vec{\beta})] = 0$$


where

- The mean of the outcome $E[Y_i]=\mu_i(\vec{\beta})$ is a function of $\vec{\beta}$
...
- The expectation of $Y_i$, $E[Y_i] = \mu_i(\beta)$ can be transformed by a link function $g(\cdot)$ so that it is *linearly* related to the covariates
$$g(\mu_i) = \boldsymbol{X}\vec{\beta}$$
...
- Assumptions: correct structural model; [independent]{.alert} observations
