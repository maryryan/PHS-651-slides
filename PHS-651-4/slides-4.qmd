...
- Want to do this with fewer distributional assumptions than GLMMs

. . .

This means we have to model (estimate) the outcome mean and the outcome
covariance [separately]{.alert}

- In GLMMs, we get these via distribution assumptions on the outcome and random effects

- How do we do this without the rigidity of distribution assumptions?

   - What if we could [empirically estimate]{.alert} the covariance structure and use that?
...
-   $\mu_{ij}$ should only depend on the covariates in the model through
    the link function $g(\cdot)$
    
    $$g(\mu_{ij}) = \vec{X}_{ij}\vec{\beta}$$

. . .

2. The covariance of the outcome $Cov[Y_{ij}, Y_{ik}|\boldsymbol{X}_i]$

- Can be broken down into 2 more components:
...
::: {.fragment .absolute bottom=400 right=100}
[Kind of like systematic variation in GLMMs!]{.alert}
:::
...
[Can we assume other types of correlation/covariance structures?]{.alert}
...
- Assumes all observations related to each other to the same degree
...
$$\begin{bmatrix}
\sigma^2_i & \alpha_{12} & \dots & \alpha_{1n} \\
\alpha_{21} & \ddots & \dots & \alpha_{2n} \\
\vdots & \dots & \ddots & \vdots\\
\alpha_{n1} & \dots & \alpha_{n(n-1)} & \sigma^2_i
\end{bmatrix}$$
...
-   Difference here: $v_i$ is the [working covariance]{.alert} for the
    ...
    -   With GLMMs, our random effects were getting stuck in with
        $\mu_{ij}(\cdot)$ and the covariance structure was induced as a
        side product
...
Since this estimating equation [only depends on the mean $\mu_{ij}(\cdot)$ and the (co)variance
$v_i(\cdot)$ functions]{.alert}, a GEE doesn't have to correspond to a full likelihood/distribution like a GLMM does
...
- Most people will automatically use the sandwich variance estimator over the model-based variance to cover their bases
...
[What abour our inference?]{.alert}
...
- The (co)variance function $v_i(\cdot)$ will likely depend on $\mu_{ij}$

   - With Normal data, $v_i(\cdot)$ was independent of $\mu_{ij}$
...
```{r cancer gee}
# run a GEE with exchangeable correlation clustered on clinicians #
...
- GEE $\beta_1$: `r round(summary(radon_gee)$coef[2,1],4)`
   ...
   - 95% CI: (`r round(summary(radon_gee)$coef[2,1] - summary(radon_gee)$coef[2,2]*pnorm(0.975), 4)`, `r round(summary(radon_gee)$coef[2,1] + summary(radon_gee)$coef[2,2]*pnorm(0.975), 4)`)
...
- GEE $e^{\beta_1}$: `r round(exp(summary(cancer_gee)$coef[2,1]),4)`
   ...
   - 95% CI: (`r round(exp(summary(cancer_gee)$coef[2,1] - summary(cancer_gee)$coef[2,2]*pnorm(0.975)), 4)`, `r round(exp(summary(cancer_gee)$coef[2,1] + summary(cancer_gee)$coef[2,2]*pnorm(0.975)), 4)`)
...
- Went from an effect size of 11.35% higher odds of remission for each extra year of experience in the GLMM to 1.21% higher odds in the GEE
