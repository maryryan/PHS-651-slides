...
- Mean: $E[Y_j] = \lambda T_j$; Variance; $Var[Y_j_j] = \lambda T$
...
$$\log(E[Y_j | \theta]) = \log[\lambda(\boldsymbol{X})T_j] = \log[\lambda(\boldsymbol{X})] + \log[T_j] = \beta_0 + U_j\beta_1 + \log(T_j)$$
...
awards_NBquad <- glmer( awards ~ write + (1 | cid),
                      family =negative.binomial(theta = awards_theta), data = awards,
                      control=glmerControl(optimizer="bobyqa") )
...
-   Because GEEs empirically estimate the variances rather than using distributional (Poisson) assumptions

-   Result: GEEs automatically account for over-dispersion without needing to use a NB
