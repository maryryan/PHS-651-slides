...
- Two ways:
   ...
   1. Adding (additional) random effects to a mixed model to induce a correlation/covariance structure
   
   2. Directly applying the correlation structure to the covariance matrix in a GEE [- we'll start here]{.fragment .alert}
...
$$g(E[Y_{ij}]) = \beta_0 + \beta_1U_{i0} + \beta_2(t_{ij}) + \beta_3(U_{i0} \times t_{ij})$$
...
$$g(Y_{ij}) = \beta_0 + \beta_1U_{i0} + \beta_2(t_{ij}) + b_{0i}Z_i+\epsilon_{ij}$$
...
$$g(Y_{ij}) = \beta_0 + \beta_1U_{i0} + \beta_2t_{ij} + b_{i0}Z_{i0} + b_{i1}t_{ij} + e_{ij}$$
...
$$g(Y_{ij}) = \beta_0 + \beta_1U_{i0} + \beta_2t_{ij} + b_{i0}Z_{i0} + b_{i1}t_{ij} + \epsilon_{ij}$$

Here we assume:

$$b_{i0} \sim N(0, \sigma^2_{b0}) ~~~~~ b_{i1} \sim N(0, \sigma^2_{b1}) ~~~~~ \epsilon_{ij} \sim N(0, \sigma^2_\epsilon)$$
...
In general, any component of $\beta$ can be allowed to vary randomly by simply including the corresponding covariate in $\vec{Z}_{ij}$
