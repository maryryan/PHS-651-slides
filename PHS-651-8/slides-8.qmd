...
- This assumes:
$$b_{i0} \sim N(0, \sigma^2_b) ~~~~~~~ \epsilon_{ij} \sim N(0, \sigma^2_{\epsilon})$$
:::
...
$$g(Y_{ij}) = \beta_0 + \beta_1U_{i0} + \beta_2t_{ij} + b_{i0}Z_{i0} + \color{green}{b_{i1}t_{ij}} + e_{ij}$$
...
$$g(Y_{ij}) = \beta_0 + \beta_1U_{i0} + \beta_2t_{ij} + b_{i0}Z_{i0} + \color{green}{b_{i1}t_{ij}} + \epsilon_{ij}$$

Here we assume:

$$b_{i0} \sim N(0, \sigma^2_{b0}) ~~~~~ \color{green}{b_{i1} \sim N(0, \sigma^2_{b1})} ~~~~~ \epsilon_{ij} \sim N(0, \sigma^2_\epsilon)$$
...
::: {.column width="50%"}
::: {.fragment}
Could also re-formulate $Cov[b_{i0}, b_{i1}]$ to be $\rho\sigma^*_{b0,b1}$, which would get us:
$$\boldsymbol{G} = \begin{bmatrix}
...
$$\text{body strength}_{ij} = \beta_0 + \beta_1(\text{Program}_i) + \beta_2t_{ij} + \beta_3(\text{Program}_i \times t_{ij}) + b_{i0}(\text{ID}_i) + b_{i1}t_{ij} + \epsilon_{ij}$$

$$\vec{b}_i \sim MVN(\vec{0}, \boldsymbol{G})$$
$$\vec{\epsilon}_i \sim MVN(\vec{0}, \sigma^2\boldsymbol{I})$$
...
- This assumes [constant variance]{.alert} across time

- Allows us to interpret $\epsilon_{ij}$ as [sampling/measurement error]{.alert}

- This is the most common type of structure to assume

. . .

Can we assume a non-independent structure for $\boldsymbol{R_i}$?
...
2. You might run into estimation issues if you try to estimate $\boldsymbol{G}$ and non-diagonal $\boldsymbol{R_i}$ without enough information in the data

3. Often random effects and regular, diagonal measurement error ($\boldsymbol{R_i} = \sigma^2_\epsilon \boldsymbol{I}$) together take care of any time-variation in correlation