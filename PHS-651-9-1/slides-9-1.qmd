...
- Include time as a variable in the mean model
...
It's important to note that while GEEs only require the mean model to be correct for $\hat{\beta}$ to be a consistent estimator (fewer distributional assumptions)...

... GLMMs require both the mean and distributional assumptions to be correct for $\hat{\beta}$ to be a consistent estimator (robust SEs can't fix everything)

---

## Inference for combinations of parameters

As we've seen the past few weeks, the introduction of time as a model covariate and and the introducton of time-by-exposure interaction effects impact the interpretation of our regression coefficients
...
- Also let $L = (0,1,0,1,0,\dots,0)$ such that $L\vec{\hat{\beta}}L= (\hat{\beta}_1 + \hat{\beta}_3)$
